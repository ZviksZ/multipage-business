...
export default class ParkingSelect {
   constructor($block) {
      this.$block = $block;
   ...
   /*refreshDrawingItems() {
      for (let i = 0; i < this.drawingItems.length; i++) {
         ...
         if (item.data('items')[this.filter]) {
            item.show()
   ...
   createPopup() {
      $('body').append(`
            <div id="popup_info" class="popup-info">
                <div class="content content-parking"></div>
            </div>
        `);
   ...
   setDataPopup(item = {}) {
      ...
      const headHTML = `
            <div class="head">
                <p class="text big text-align-left">${nameType} ${name}</p>
      ...
      apartmentsHTML += `
                <div class="item item-parking">
                    <span class="text mb">${area} м<sup>2</sup></span>          
                    <span class="text text-red">${price} ₽ </span>                    
                </div>
            `;
